...
import (
	...
	"github.com/ducklawrence05/go-test-backend-api/internal/service/user"
...
func (uh *UserHandler) Register(c *gin.Context) {
	...
	if err := uh.userService.Register(payload); err != nil {
...
func (uh *UserHandler) Login(c *gin.Context) {
	...
	accessToken, refreshToken, err := uh.userService.Login(payload)
	if err != nil {
		c.JSON(err.StatusCode, gin.H{"error": err.Msg})
		return
	}
...
func (uh *UserHandler) Logout(c *gin.Context) {
	...
	err := uh.userService.Logout(userID.(uuid.UUID), payload)
	if err != nil {
		c.JSON(err.StatusCode, gin.H{"error": err.Msg})
		return
	}
...
func (uh *UserHandler) UpdateMe(c *gin.Context) {
	...
	user, err := uh.userService.UpdateMe(userID.(uuid.UUID), payload)
	if err != nil {
		c.JSON(err.StatusCode, gin.H{"error": err.Msg})
		return
	}
...
func (uh *UserHandler) ChangePassword(c *gin.Context) {
	...
	if err := uh.userService.ChangePassword(userID.(uuid.UUID), payload); err != nil {
		c.JSON(err.StatusCode, gin.H{"error": err.Msg})
		return
	}
...
func (uh *UserHandler) RefreshToken(c *gin.Context) {
	...
	accessToken, refreshToken, err := uh.userService.RefreshToken(payload)
	if err != nil {
